...
from app.common.errors import IncorrectDataRecivedError, ReqFieldMissingError, ServerError
from app.common.decos import log
from app.common.meta import ClientVerifier
from app.db.client_db import ClientDatabase
...
def contacts_list_request(sock, name):
    ...
    if RESPONSE in ans and ans[RESPONSE] == 202:
        ...
        raise ServerError('Ошибка запроса контакт листа')
...
def add_contact(sock, username, contact):
    ...
    if RESPONSE in ans and ans[RESPONSE] == 200:
        ...
        raise ServerError('Ошибка добавления контакта в список контактов')
...
def user_list_request(sock, username):
    ...
    if RESPONSE in ans and ans[RESPONSE] == 202:
        ...
        raise ServerError('Ошибка получения всех контактов')
...
def main(server_address=None, server_port=None, client_name=None):
    # Сообщаем о запуске
    print('Консольный месседжер. Клиентский модуль.')

    # Загружаем параметы коммандной строки
    if not server_address and not server_port and not client_name:
    ...
    try:
        ...
        database = ClientDatabase(
            create_sqlite_uri(f'../db/client_{client_name}.db3')
        )
